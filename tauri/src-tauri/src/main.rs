...
//! The Paint desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Paint does — the
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if paint_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Paint could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Paint desktop shell could not start");
}
