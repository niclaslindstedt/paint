...
//! Answering the `paint://` scheme — the effects half of
//! `paint_shell::webroot`, which owns every decision this file acts on.
//!
...
//!
//! `PAINT_WEBROOT` overrides both, which is what lets a build serve a site from
//! somewhere else without recompiling — the same escape hatch `PAINT_APP_URL`
//! gives for a REMOTE site.

use std::fs;
use std::path::PathBuf;

use paint_shell::webroot::{content_type_for, resolve_webroot_file};
...
pub fn webroot_dir(app: &AppHandle) -> PathBuf {
    if let Some(override_dir) = std::env::var_os("PAINT_WEBROOT") {
...
/// returns a body, not a stream, and the largest thing a Paint build contains
