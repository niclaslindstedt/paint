...
//! Every decision about that file is `paint_shell::window_state`'s; this
//! file only asks the window and the monitors.

use paint_shell::config::{
    app_origin, is_internal_url, remote_app_url, start_url, BRAND_BG, WINDOW_TITLE,
};
use paint_shell::window_state::{
...
const DEFAULT_WINDOW: WindowState = paint_shell::window_state::DEFAULT_STATE;
