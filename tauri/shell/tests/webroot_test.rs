...
impl Site {
    fn new(name: &str) -> Self {
        let root = std::env::temp_dir().join(format!("paint-webroot-{name}"));
...
fn a_symlink_out_of_the_webroot_is_refused() {
    ...
    let outside = std::env::temp_dir().join("paint-webroot-symlink-outside.txt");
...
fn an_unknown_extension_is_not_guessed_at() {
    assert_eq!(
        content_type_for(Path::new("drawing.pct")),
