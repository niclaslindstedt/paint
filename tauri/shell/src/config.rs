...
//! shipping: `PAINT_APP_URL` points the window at a remote URL instead (the
//! `/preview/` deploy slot, say), and `PAINT_WEBROOT` serves a different
...
/// IndexedDB unable to keep a drawing between launches. A registered scheme
/// gives one stable origin that the drawings are keyed to for the life of the
/// install.
pub const APP_SCHEME: &str = "paint";
...
/// CONSTANT — the origin is what the user's drawings are keyed to, so changing
...
pub const WINDOW_TITLE: &str = "Paint";

/// The window's opening size, and the smallest it may be dragged to.
///
/// The floor is the narrowest the toolbar and the canvas are both usable at —
/// below it the app is not broken, it is just a worse phone.
