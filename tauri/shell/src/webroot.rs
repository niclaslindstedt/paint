...
//! wrong to simplify.
//!
//! The site is served from a registered private scheme (`paint://`, see
...
//! to lose:
//!
//!  1. **One stable origin.** The user's drawings live in IndexedDB and their
//!     settings in `localStorage`, both keyed by origin. `paint://` is a
//!     constant, so a sketchbook survives every update. A `file://` page (or a
//!     server on an ephemeral port) would hand the user a different origin —
//!     and an empty sketchbook — at some point.
//!
...
/// `paint:///C:/…` must not become a drive letter.
