...
A desktop wrapper around Paint for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Paint does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `paint://` off the bundled `webroot/`               |
...
`cargo test -p paint-shell` runs the entire decision layer on a machine with a
...
**The origin is the one thing to be careful with.** The user's drawings live in
...
`file://` page is an opaque origin, which would leave the sketchbook unable to
survive a launch.
...
| Variable        | Effect                                                               |
| --------------- | -------------------------------------------------------------------- |
| `PAINT_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `PAINT_WEBROOT` | Serve the site from somewhere else without rebuilding                |
