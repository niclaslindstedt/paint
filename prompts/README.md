...
| Prompt             | Used by                                                                  |
| ------------------ | ------------------------------------------------------------------------ |
| `describe-drawing` | Not wired into the app yet — the prompt behind planned drawing alt text. |

The app itself performs **no LLM calls at runtime**: Paint is a frontend-only,
local-first PWA with no server to hold an API key. A prompt lands here first,
versioned, and is only then wired into whatever surface calls it.
