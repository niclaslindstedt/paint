...
export function renderDrawing(
  ...
  const marks = visibleStrokes(drawing, scope);
  if (anyLayerFiltered(drawing) && !options.unfiltered) {
    // The stack does its own — a rubbing out on a filtered layer is scoped to
    // that layer's surface, so what it owes is scoped to it too.
    paintStack(ctx, drawing, scope, options);
  } else {
    paintStrokes(ctx, marks, options);
  ...
  if (draft) {
    ...
    relayFixed(ctx, [draft], options, marks);
  }
...
function paintStack(
  ...
  // What a rubbing out on an unfiltered layer has to put back: everything
  // already painted, whatever layer it was on — because that is exactly what it
  // just took off. A filtered layer's own is settled inside its surface.
  const under: Stroke[] = [];
