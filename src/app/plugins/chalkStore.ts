...
// The chalk marks that have already landed.
//
// The pencil's store (`leadStore.ts`), one shelf along, for the same reason: a
// repaint asks for every mark on the page again, and a landed chalk mark is a
// *field* — a board scrubbed by a stick, worked out cell by cell and written
// through `putImageData` — that would otherwise be worked out again on every
// one of them. Each mark dries once per view and is blitted for as long as the
// page keeps asking for the same picture.
//
// A chalk field is worked at the *device's* pitch (see `chalkSim.ts`), so the
// cell is part of the ask and a zoom that settles at a new scale dries every
// mark again, once — the pencil's own trade, made for the pencil's reason:
// grain that is always as fine as the screen can show.
//
// The rules are the lead store's: refuse-not-evict when full, a `WeakRef`
// sweep of paths the rest of the app has let go of, and marks compared by the
// identity of their points. There is no turned-away slot because a chalk mark
// is dry the moment it lands: nothing asks for one twice in a frame.
...
/** The most landed marks held, and the most cells between them. A board of
 *  chalk writing is many small marks, so the mark count is generous and the
 *  cell budget is what actually binds. */
 ...
 *  chalk is a colour at an alpha, not a glaze, so the page shows through the
 *  compositing rather than through the field. */
export type Ask = {
  points: readonly Point[];
  size: number;
  /** How hard the hand was bearing down — the chalk's one axis (see
   *  `PRESSURE` in `builtin/dials.ts`). */
  press: number;
  ground: GroundProfile;
  color: string;
  /** The pitch the field was worked at — the input that carries the zoom. */
  cell: number;
};
...
export function forgetChalkStore(bounds?: {
  marks?: number;
  cells?: number;
}): void {
  kept.length = 0;
