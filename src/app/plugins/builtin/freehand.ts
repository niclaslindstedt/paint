...
// differ only in their ink (colour source, width multiplier, opacity), so they
// share one behaviour factory — the clearest demonstration of what a tool
// plugin actually has to supply.

import type { Point } from "../../types.ts";
import { applyInk, distance, paintPath } from "../ink.ts";
...
export function freehandBehaviour(ink: FreehandInk = {}): ToolBehaviour {
  ...
  return {
    ...
    paint: (ctx2d, stroke) => {
      ...
      paintPath(ctx2d, stroke.shape.points, stroke.size);
    },
  };
}
